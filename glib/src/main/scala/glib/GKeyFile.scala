...
final class GKeyFile extends GAllocated {
   ...
   */
  @inline def loadFromFile(file: CString, flags: GKeyFileFlags)(implicit error: ResultPtr[GError]): gboolean = extern
  @inline def loadFromFile(file: String, flags: GKeyFileFlags = GKeyFileFlags.NONE): Try[GKeyFile] = Zone{ implicit z =>
    implicit val err = GError.alloc
    loadFromFile(toCString(file),flags)
    if(err.isDefined) Failure(new RuntimeException(err.wrappedValue.message)) else Success(this)
  }
...
//
   ...
   */
  @inline def saveToFile(filename: CString)(implicit error: ResultPtr[GError]): gboolean = extern
  def saveToFile(filename: String): Try[Unit] = Zone{ implicit z =>
    implicit val err = GError.alloc
    saveToFile(toCString(filename))
    if(err.isDefined)
      Failure(new RuntimeException(err.wrappedValue.message))
    else
      Success(())
  }
//
...
//
   ...
   * @param error return location for a GError, or null
   */
  @inline def hasKey(groupName: CString, key: CString)(implicit error: ResultPtr[GError]): gboolean = extern
  def hasKey(groupName: String, key: String): gboolean = Zone{ implicit z =>
    hasKey(toCString(groupName), toCString(key))(null)
  }
...
//
   ...
   * @param error return location for a GError, or null
   */
  @inline def getString(groupName: CString, key: CString)(implicit error: ResultPtr[GError]): CString = extern
  def getString(groupName: String, key: String): Option[String] = Zone{ implicit z =>
    implicit val err = GError.alloc
    val res = getString(toCString(groupName),toCString(key))
    if(res == null) None else Some(fromCString(res))
  }
   ...
   */
  @inline def setString(groupName: CString, key: CString, string: CString): Unit = extern
  def setString(groupName: String, key: String, string: String): Unit = Zone{ implicit z =>
    setString(toCString(groupName),toCString(key),toCString(string))
  }
...
object GKeyFile {
  ...
  @name("g_key_file_new")
  def apply(): GKeyFile = extern

  def fromFile(filename: String, flags: GKeyFileFlags = GKeyFileFlags.NONE): Try[GKeyFile] =
    apply().loadFromFile(filename,flags)
}
